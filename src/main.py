...
from ui.components import MessageBubble, ModelSelector, AuthorizationWindow  # Компоненты пользовательского интерфейса
...
class ChatApp:
    ...
    def update_balance(self):
        ...
        try:
            ...
            # Проверка баланса после  обновления 
            if float(balance)<10:
                notify_admin(
                    subject="Внимание! Низкий баланс",
                    message=f"Текущий баланс: {balance}. Пожалуйста, пополните счет."
                )
    ...
    def main(self, page: ft.Page):
        ...
        def on_auth_success(self, api_key):
            self.api_key = api_key
            print(f"Успешный вход с ключом: {api_key}")
            # Здесь можно заменить экран авторизации на основной чат
            self.page.controls.clear()  # Очистить текущий UI
            self.build_chat_ui()         # Построить UI чата
            self.page.update()
        
        auth_window = AuthorizationWindow(on_auth_success)
        page.controls.clear()
        page.add(auth_window)
        page.update() 
